...
var entities = {};
var entityList = [];
var sceneFile = "/static/scenes/umn.json";
var sceneModel = "/static/assets/model/umn.obj";
var sceneTexture = "/static/assets/texture/umn.png";
...
$( document ).ready(function() {
  ...
  $("#map").append('<circle id="dest" cx="'+ $("#map").width()/2 +'" cy="'+ $("#map").height()/2 +'" r="10" stroke="orange" stroke-width="4" fill="blue" />');
  $("#map").html($("#map").html());
  ...
  $('#map').click( function(e){
    ...
    let x = 1.0*posX/$("#map").width();
    let z = 1.0*posY/$("#map").height();
    x = x*(1564.8457202490636+1420.0006612181908)-1420.0006612181908;
    z = z*(890.9378900451509+727.277163868663)-727.277163868663;

    var model = entities[0];
    sx = entityPos[0][0]+1;
    sy = entityPos[0][1]+1;
    sz = entityPos[0][2]+1;

    // -3081.614746	0	-1803.562744
    //3765.580322	286.9198	8874.74707
    

    const myHeaders = new Headers();
    myHeaders.append("Content-Type", "application/json");
    const raw = JSON.stringify({"algorithm": $("#algSelect").val(), "start": [sx,sy,sz],"dest": [x,sy,z]});
...
function updateModel(id, newPos, newDir) {
  var model = entities[id];
  model.position.x = newPos[0];
  model.position.y = newPos[1];
  model.position.z = newPos[2];
  ...
  var dir = new THREE.Vector3(newDir[0], newDir[1], newDir[2]);
  var pos = new THREE.Vector3();
  pos.addVectors(dir, model.position);
  ...
  $.getJSON(sceneFile, function(json) {
    ...
    for (var i = 0; i < json.length; i++) {
      ...
      if (command.command == "CreateEntity") {
        ...
        if (command.params.type == "drone") {
          addEntity({id: 0, details: command.params});
        }
        else {
          addEntity({id: 1, details: command.params});
        }
...
function addPath(id, path) {
  ...
  if (id in paths) {
    scene.remove(paths[id]);
    delete paths[id];
    delete pathPoints[id];
  }

  let end = path[path.length-1]
  updateModel(1, [end[0] + 20, end[1], end[2] + 20],[1,0,0])
  ...
  loader2.load(
    ...
    function ( object ) {
      ...
      if (obj.type == "route") {
        ...
        //addPath([[500.36056,225,70], [-100,257.0656264,100], [-200.9930709,253.9161185,345]])
      }
        models.push(object);
      scene.add( object );
    },
...
const onLoad = ( gltf, position, scale, start, duration, details, id ) => {
  ...
  $("#entitySelect option[value='0']")[0].selected = true;
  //$("#entitySelect").val(0);
  changeView();
};
...
function addEntity(data) {
  ...
  entityPos[data.id] = data.details.position;
}
...
function update() {
  ...
  if (0 in entities) {
    var model = entities[0];
    if (0 in pathPoints && pathPoints[0].length > 0) {
      let pos = entityPos[0];
      let point = pathPoints[0][0];
      let dir = [point[0] - pos[0], point[1] - pos[1], point[2] - pos[2]];
      let size = Math.sqrt(dir[0]*dir[0] + dir[1]*dir[1] + dir[2]*dir[2]);
      if (size <= 3.0) {
        pathPoints[0].splice(0,1);
      }
      dir = [dir[0]/size, dir[1]/size, dir[2]/size];
      //console.log(dir);
      for (let i = 0; i < 3; i++) {
        pos[i] = pos[i] + 15.0*dir[i]*delta*simSpeed;
      }
      entityPos[0] = pos;
      updateModel(0, pos, dir);
    }
    else {
      //addPath(0, [[500.36056,225,70], [-100,257.0656264,100], [-200.9930709,253.9161185,345]]);
    }
  }
  else {
  }
